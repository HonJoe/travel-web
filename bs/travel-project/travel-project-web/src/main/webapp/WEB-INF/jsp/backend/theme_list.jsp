...
<section class="rt_wrap content mCustomScrollbar">
    <div class="rt_content">
        <div class="page_title">
            <h2 class="fl">主题列表</h2>
            <a href="http://localhost:8080/manager/theme/addView" class="fr top_rt_btn add_icon">添加主题</a>
        </div>
...
<script>
    function init(){
        $.ajax({
            url:"http://localhost:8080/theme/list",
            async: false,
            type:"get",
            dataType:"json",
                        ...
                        $.ajax({
                            ...
                            success:function(result){
                                ...
                                $.each(data.records,function(i,item){
                                    var edtior="<td class='center'><a href='http://localhost:8080/manager/theme/detailView/"+item.id+"' title='编辑' class='link_icon' >&#101;</a><a  title='删除' class='link_icon'value='"+item.id+"' onclick='del(this)'>&#100;</a></td>";
                                    showTableHtml += "<tr>"+
                                        "<td style=\"padding-left: 20px;\">"+item.themeName+"</td>"+
                                        "<td style=\"padding-left: 20px;\">"+item.detail+"</td>"+
                                        edtior+"</tr>";
                                });
    ...
    function del(obj) {
        ...
        if(confirm("确实要删除吗？")){
            $.ajax({
                type:"GET",
                url:"http://localhost:8080/manager/theme/delete/"+id,
                dataType:"json",
